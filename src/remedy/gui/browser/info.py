...
class InfoPanel(QWidget):
    def __init__(self, index, *args, **kwargs):
        ...
        titlebox = (
            QHBoxLayout()
        )  # this is just to disable title shrinking when wrapping text
    ...
    def _addDetailRow(self, title, data, kerning=True):
        ...
        f.setBold(True)
        t.setFont(f)
        t.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignTop)
        ...
        if isinstance(data, str):
            ...
            if not kerning:
                ...
                f.setKerning(False)
                t.setFont(f)
            t.setFlags(Qt.ItemFlag.ItemIsEnabled | Qt.ItemFlag.ItemIsSelectable)
            t.setTextAlignment(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop)
            ...
            self.details.verticalHeader().setSectionResizeMode(
                n, QHeaderView.ResizeToContents
            )
    ...
    def setEntries(self, entries=None):
        if entries is None or len(entries) == 0:
            ...
            if pdfs:
                self._addDetailRow('PDFs', str(pdfs))
            if ebooks:
                self._addDetailRow('EBooks', str(ebooks))
            if notebs:
                self._addDetailRow('Notebooks', str(notebs))
            self._finalizeDetails()
    ...
    def setEntry(self, entry=None):
        ...
        if isinstance(entry, PDFBasedDoc):
            ...
            if np:
                self._addDetailRow('Marked pages', str(np))
            np = entry.numHighlightedPages()
            if np:
                self._addDetailRow('Highl. pages', str(np))
            if entry.documentMetadata:
                self._addDetailRow('Title', entry.documentMetadata.get('title'))
                self._addDetailRow(
                    'Authors', ','.join(entry.documentMetadata.get('authors', []))
                )
        ...
        if tags:
            ...
            h = (
                taglist.visualRect(
                    taglist.model().index(taglist.model().rowCount() - 1, 0)
                ).bottom()
                + 4
            )
