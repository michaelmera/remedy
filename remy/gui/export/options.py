...
from remy.remarkable.export import (
    validatePageRanges,
    parseExcludeLayers,
...
class ExportDialog(QDialog):
    ...
    def getFileExportOptions(parent=None, options={}, **kwargs):
        d = ExportDialog(
            mode=ExportDialog.FileExport, options=options, parent=parent, **kwargs
        )
        res = d.exec_()
    ...
    def __init__(self, filename=None, options={}, mode=None, **kwargs):
        ...
        act = pathsel.addAction(
            QIcon(":assets/symbolic/folder.svg"), QLineEdit.TrailingPosition
        )
        act.setToolTip("Change...")
        act.triggered.connect(self.selectPath)

        act = pathsel.addAction(
            QIcon(":assets/symbolic/warning.svg"), QLineEdit.TrailingPosition
        )
        ...
        act = pageRanges.addAction(
            QIcon(":assets/symbolic/info.svg"), QLineEdit.TrailingPosition
        )
        act.setToolTip(
            "Examples:\nList of pages: 1,12,3\nRange: 3:end\nMixed: 5,1:3,end\nReverse order: end:1:-1\nSkipping even pages: 1:end:2"
        )
        act = pageRanges.addAction(
            QIcon(":assets/symbolic/error.svg"), QLineEdit.TrailingPosition
        )
    ...
    def selectPath(self, *args):
        if self.mode == ExportDialog.FileExport:
            filename, ok = QFileDialog.getSaveFileName(
                self, "Export PDF...", self.pathsel.text(), "PDF (*.pdf)"
            )
        else:
            filename, ok = QFileDialog.getExistingDirectory(
                self, "Export PDF...", self.pathsel.text()
            )
        if ok and filename:
